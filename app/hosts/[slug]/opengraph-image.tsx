...
import { getHostBySlug, HOSTS } from "@/lib/constants";
import { renderOGCard, OG_SIZE, OG_CONTENT_TYPE } from "@/lib/og-card";

/**
 * Per-host Open Graph image (`/hosts/<slug>/opengraph-image`).
 *
 ...
 * `generateMetadata`. Scrapers requesting `/hosts/adam` see the OG tag
 * point here, fetch this route, and get back the rendered 1200×630 PNG.
 *
 * Static generation (not edge runtime): we pair `generateStaticParams`
 * with Node.js runtime (the default) so every host card is rasterized to
 * a static PNG at build time. Trade-offs:
 *   - Edge runtime is incompatible with `generateStaticParams` in Next 15.
 *   - For a fixed set (3 hosts) static generation is strictly better:
 *     zero cold-start, served directly from the CDN.
 *
...
export const alt = "Longevity Leadership Conference host card";
...
export default async function Image({
  ...
  // Resolve the host image to an absolute URL. At request-time the edge
  ...
  return new ImageResponse(
    renderOGCard({
      ...
      kicker: "Your Host",
      imagePosition:
        "imagePosition" in host && typeof host.imagePosition === "string"
          ? host.imagePosition
          : "center 20%",
    }),
    size,
